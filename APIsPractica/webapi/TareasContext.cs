...
public class TareasContext : DbContext // El contexto siempre va a heredar de esto
{
	// Al dbcontext se le agrega la config de las colecciones que vamos a utilizar que son las de abajo, estas colecciones se convertiran en una tabla en la bd al momento que nos conectemos
	...
	// En OnModelCreating tenemos toda la config de las tablas, este modelo se conoce como FLUENT API, que es la config de la bd utilizando diferentes metodos de extension que tiene la funcionalidad de EF que es FA  
	...
	protected override void OnModelCreating(ModelBuilder modelBuilder)
	{
		...
		// Los dos datos que se estan creando para categoria
		...
		modelBuilder.Entity<Categoria>(categoria =>
		{
			...
			// has data permite recibir datos iniciales
        ...
        modelBuilder.Entity<Tarea>(tarea =>
		{
			...
			// tarea tiene categorias que pueden contener multiples tareas con with many y a su vez decimos que tiene una clave foranea
			// Permite configurar la clave foranea entre la relacion de tareas y categorias
            tarea.Property(p => p.Titulo).IsRequired().HasMaxLength(200); 
            tarea.Property(p => p.Descripcion).IsRequired(false);
            tarea.Property(p => p.PrioridadTarea);
            tarea.Property(p => p.FechaCreacion);
			tarea.HasData(tareasInit); // Para agregar datos iniciales
